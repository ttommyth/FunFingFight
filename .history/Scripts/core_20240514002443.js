		...
		if(preCreateUnit !=null){
			if(money-preCreateUnit.cost>=0){
                if (checkValidMouseArea(mousePos)){
                    ...
                    //TODO: create unit done, report to server
...
function addNatureUnit(unit, tellClient = isHost) {
    if (tellClient) {
        console.log([unit.constructor.name, unit.pos, unit.team]);
        room.server.addNatureUnit(unit.constructor.name, JSON.stringify(unit.pos), ""+unit.team);
    } else {
...
function addTeamUnit(unit, tellClient = true) {
    if (unit instanceof Projectile_Prototype)
        tellClient = false;
    if (tellClient || (tellClient && !isHost  && unit instanceof Unit_Prototype)) {
        console.log([unit.constructor.name, unit.pos, unit.team]);
        room.server.addTeamUnit(unit.constructor.name, JSON.stringify(unit.pos),""+ unit.team);
    } else {
        allTeamsArr.push(unit);
...
//////////////////////////////////////////
var isHost = true;
var room = $.connection.GameHub;
var playerNum = 2;

$(function () {
    room.client.addNatureUnit = function (type, pos, team) {
        console.log("Add " + type + " in " + pos + " for " + team);
        var posObject = JSON.parse(pos);
        var teamNum = parseInt(team);
        //var unit = new window[""+type](posObject, teamNum);
        //var unit = new types[type].constructor(posObject, teamNum);
        var unit = new (eval(type))(posObject, teamNum);
        addNatureUnit(unit, false);
    };


    room.client.addTeamUnit = function (type, pos, team) {
        console.log("Add " + type + " in " + pos + " for " + team);
        var posObject = JSON.parse(pos);
        var teamNum = parseInt(team);
        //var unit = new window[""+type](posObject, teamNum);
        //var unit = new types[type].constructor(posObject, teamNum);
        var unit = new (eval(type))(posObject, teamNum);
        addTeamUnit(unit, false);
    };

    room.client.newPlayer = function (count) {
        ...
        if (count == 1) {
            ...
            if (playerNum == 2) {
                isHost = false;
                currentTeam = RedTeamNum;
                checkValidMouseArea = leftPlayerCheck;
            } else {

                checkValidMouseArea = rightPlayerCheck;
            }
            gameStart();
        }
    }
